...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'6hnlIBcmYycvbWU',
      'username':'Hardy.Kuhic44',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Hardy.Kuhic44' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'POhlCHrR_aAKYN4',
      'username':'Caroline87',
      'isDeleted':false,
      'isActive':true,
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Caroline87' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
